...
import { useClientContentDetailsContext } from "./ClientDetailsContentContext";

export default function ClientDetailsContent({ handleModalClientsEdit }) {
  const { client, cobrancas } = useClientContentDetailsContext();
          ...
          {cobrancas.length === 0 ? (
            ...
            <ul>
              {cobrancas.map((cobranca) => (
                <li key={cobranca.id}>
                  {cobranca.descricao} - R$ {(cobranca.valor / 100).toFixed(2)}{" "}
                  - {cobranca.status}
                </li>
              ))}
            </ul>
